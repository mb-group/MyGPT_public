...
backend_container_id=$(docker ps -aqf "ancestor=ghcr.io/mb-group/mygpt-backend:latest")

if [ -z "$backend_container_id" ]; then
	backend_container_id=$(docker ps -aqf "ancestor=ghcr.io/mb-group/mygpt-backend:arm64")
	if [ -z "$backend_container_id" ]; then
